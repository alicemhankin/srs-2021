...
samples_per_tree = 10              #number of pairs of points we sample from each tree
no_of_trees = 300
height_of_each_tree = 60           #which generation we sample from
  ...
  while (done != 1){
    ...
    for(i in 1:height_of_each_tree){ 
      if (length(vec)!=0){
        ...
        #finds next gen given this gen (uses ulam-harris labeling)
        for (j in vec){
          ran = rgeom(1,param)
          if (ran!=0){
            for (k in 1:ran){
